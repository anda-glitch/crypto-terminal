...
Task: Provide an ultra-condensed, highly technical 3-sentence market bias evaluation for high-frequency day traders. Declare an overarching bias (BULLISH, BEARISH, or NEUTRAL). Keep it clinical and data-driven."""
    ...
    if not ollama_ok():
        return jsonify({"status": "error", "message": "AI Bridge not active. Ensure Ollama is running on your Mac Mini."}), 503
    ...
    try:
        resp = requests.post(OLLAMA, json={
            ...
            "prompt": prompt,
            "stream": False
        }, headers=get_cf_headers(), timeout=15)
        ...
        return jsonify({"status": "error", "message": f"AI Bridge connection failed: {str(e)}"}), 503
